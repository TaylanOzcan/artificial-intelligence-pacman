...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        allPossible = util.Counter()  # All beliefs is set to 0 by default
        legalPositions = self.legalPositions  # Get all legal positions

        if noisyDistance is None:  # If the ghost has been eaten
            # Set the belief for jail position to 1 (All others are 0)
            allPossible[self.getJailPosition()] = 1.0
        else:  # If ghost has not been eaten
            for p in legalPositions:  # For each legal position
                # Find the manhattan distance from pacman to position
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        "*** YOUR CODE HERE ***"
        allPossible = util.Counter()  # All beliefs is set to 0 by default
        for oldPosition in self.legalPositions:  # For each legal position (they are old positions now)
            # Calculate the new position distribution for old position
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPosition))
            for newPosition, probability in newPosDist.items():  # For each item in new position distribution
                # Sum out old position
                allPossible[newPosition] += probability * self.beliefs[oldPosition]

        # Normalize and update the beliefs
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        particles = []  # Initialize the list of particles
        numLegalPositions = len(self.legalPositions)  # Get the number of positions
        quotient = self.numParticles / numLegalPositions  # Get the quotient of the division
        remainder = self.numParticles % numLegalPositions  # Get the remainder of the division

        for i in range(quotient):  # Fill particles list by appending legal positions repeatedly (It is uniform)
            particles.extend(self.legalPositions)

        if remainder > 0:  # If there is a remainder bigger than 0
            # Fill the remaining slots
            particles.extend(self.legalPositions[0:remainder])
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:  # If the ghost has been eaten
            ...
            # Set all particles to jail position
            self.particles = [jailPosition] * self.numParticles
        else:  # If ghost has not been eaten
            for particle in self.particles:  # For each particle
                # Increase particle's weight
                allPossible[particle] += 1

            for position in allPossible:  # For each position in the belief distribution
                # Find the manhattan distance from pacman to position
                trueDistance = util.manhattanDistance(position, pacmanPosition)
                # Update the belief for the position
                allPossible[position] *= emissionModel[trueDistance]

            if sum(allPossible.values()) is 0:  # If all weights are equal to 0
                self.initializeUniformly(gameState)  # Reinitialize particles uniformly
            else:
                # Resample particles
                self.particles = [util.sampleFromCounter(allPossible) for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Get the positions from previous step
        oldPositions = self.particles
        # Update the particles by sampling from the distribution formed by using old positions
        self.particles = [util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, oldPos)))
    ...
    def getBeliefDistribution(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        beliefDist = util.Counter()  # Initialize the belief distribution
        for particle in self.particles:  # For each particle (position)
            beliefDist[particle] += 1  # Increase the weight of the relevant position in the distribution
        beliefDist.normalize()  # Normalize the distribution
        return beliefDist
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        particles = []  # Initialize an empty particles list
        positions = self.legalPositions  # Get the legal positions
        permutations = list(product(positions, repeat=self.numGhosts))  # Get all permutations of legal positions
        shuffle(permutations)  # Shuffle the permutations of legal positions

        quotient = self.numParticles / len(permutations)  # Get the quotient of the division
        remainder = self.numParticles % len(permutations)  # Get the remainder of the division

        # Fill particles list by appending the permutations of legal positions repeatedly (It is uniform)
        for i in range(quotient):
            particles.extend(permutations)

        if remainder > 0:  # If there is a remainder bigger than 0
            # Fill the remaining slots
            particles.extend(permutations[0:remainder])
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        eatenIndices = []  # Initialize a list to track the indices of eaten ghosts

        for i in range(self.numGhosts):  # For each ghost
            allPossible = util.Counter()  # Initialize a belief (counter) object for the specific ghost
            if noisyDistances[i] is None:  # If the ghost has been eaten
                #  Update the particles by changing the specific ghost's positions to jail
                self.particles = [self.getParticleWithGhostInJail(particle, i) for particle in self.particles]
                eatenIndices.append(i)  # Add the ghost's index into eaten ghost indices list
            else:  # If ghost has not been eaten
                beliefDist = self.getBeliefDistribution()  # Get the old belief distribution
                emissionModel = emissionModels[i]  # Get the emission model for the specific ghost
                for particle in self.particles:  # For each particle
                    trueDistance = util.manhattanDistance(particle[i], pacmanPosition)  # Calculate the true distance
                    # Update belief for the particle
                    allPossible[particle] = beliefDist[particle] * emissionModel[trueDistance]

                if sum(allPossible.values()) is 0:  # If all weights are equal to 0
                    self.initializeParticles()  # Reinitialize particles uniformly
                    for eatenIndex in eatenIndices:  # For each eaten ghost
                        #  Update the particles by changing the specific ghost's positions to jail
                        self.particles = \
                            [self.getParticleWithGhostInJail(particle, eatenIndex) for particle in self.particles]
                else:
                    allPossible.normalize()  # Normalize the particle distribution
                    #  Resample particles
                    self.particles = [util.sampleFromCounter(allPossible) for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):  # For each ghost
                #  Update the particle by sampling from the distribution formed by using old positions
    ...
    def getBeliefDistribution(self):
        ...
        beliefDist = util.Counter()  # Initialize the belief distribution
        for particle in self.particles:  # For each particle
            beliefDist[particle] += 1  # Increase the weight of the relevant particle in the distribution
        beliefDist.normalize()  # Normalize the distribution
        return beliefDist
